...
int runLuaGame::l_cls(lua_State* L) {
  ...
  if(cn != NULL)
  {
    tft.fillRect(0, 0, TFT_WIDTH, TFT_HEIGHT, gethaco3Col(cn));
  }else{
    tft.fillRect(0, 0, TFT_WIDTH, TFT_HEIGHT, gethaco3Col(0));
  }
...
int runLuaGame::l_btnp(lua_State* L)
{
  ...
  if(buttonState[n] < 25){//buttonState.size()
...
int runLuaGame::l_appmode(lua_State* L){//ファイル名を取得して、そのファイルを実行runする
  runLuaGame* self = (runLuaGame*)lua_touserdata(L, lua_upvalueindex(1));
  const char* file = lua_tostring(L, 1);
  const uint8_t modeno = lua_tointeger(L, 2);
