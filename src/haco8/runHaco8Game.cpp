...
// extern void readMap(const String& _appfileName, int wx, int wy);
...
bool readRowFromBinary(const char *filename, size_t startRow, std::vector<uint8_t> &output) {
  ...
  while (1) {
    ...
    size_t bytesRead = file.read(&compressedData, sizeof(unsigned char));
    ...
    if (bytesRead != 1) {
      ...
      sprno = compressedData >> 5;
      repeatnum = compressedData & 0b00011111;

    // if(RowCursor == startRow+32)break;//３２個の値（１６行）分break
    if (startRow>nnum) {
      //スタート位置までスルーする

    }else if (startRow<=nnum) {
      if(nnum==startRow+16)break;
      if (sprno == 0 && repeatnum == 0) {
        ...
        output.push_back(sprno);
        output.push_back(repeatnum);

    }
    if (sprno == 3 && repeatnum == 31) {//01111111 
      nnum++;
      Serial.println("");
    }
...
// size_t upspritenos[20];
// size_t preupspritenos[20];
// size_t movx=0;
// movx++;//波の動き
// movx%=8;
...
void printMapArray(String _mapFileName, int wx, int wy) {
  ...
  // Specify the row number to read from the binary data
  // size_t rowNum = wy;
  ...
  if (readRowFromBinary(_mapFileName.c_str(), startRow, rowData)) {
      ...
      if (sprno == 3 && repeatnum == 31) {//終端条件のみ
        ...
        for (int y = 0; y < 8; y++) {
          for (int x = 0; x < 8; x++) {
            ...
            // if(sprno==0){sprite88_roi.drawPixel((x+frame)%8, y, getcno2tftc(bit4));}//海の時
            // else{
              sprite88_roi.drawPixel(x, y, getcno2tftc(bit4));
              // }
        ...
        for (int n = 0; n < repeatnum; n++) 
        {
          ...
          if (sprx >= wx && sprx < wx + 20) {
            ...
            if(displaysprx!=0){
              if(presprno==0 && sprno!=0){//海から陸ならば
                tft.fillRect((ColCursor + n - wx - 1) * 8+7, j * 8, 1,8, RunHaco8Game::HACO3_C7);
              }
              if(presprno!=0 && sprno==0){//陸から海ならば
                tft.fillRect((ColCursor + n - wx) * 8, j * 8, 1,8, RunHaco8Game::HACO3_C7);
              }
            }
            ...
            if(j!=0){
              if(sprno!=0&&mapArray[j-1][displaysprx]==0){//陸の時上が海か
                // sprite88_roi.pushSprite(&tft, (ColCursor + n - wx) * 8, j * 8);
                tft.fillRect((ColCursor + n - wx) * 8, (j-1) * 8 + 7, 8,1, RunHaco8Game::HACO3_C7);
              }

              if(sprno==0&&mapArray[j-1][displaysprx]!=0){//海の時上が陸か
                // sprite88_roi.pushSprite(&tft, (ColCursor + n - wx) * 8, j * 8);
                tft.fillRect((ColCursor + n - wx) * 8, j * 8, 8,1, RunHaco8Game::HACO3_C7);
              }
          ...
          // memcpy(preupspritenos, upspritenos, sizeof(upspritenos));
    ...
    Serial.println("Error reading row from binary data");
  }
  // `memcpy`関数を使用した配列のコピー
...
int RunHaco8Game::l_rmap(lua_State* L){
  ...
  mapFileName = fn;
  readMap(mapFileName,wx,wy);//yを先にする
...
// SPIFFS からマップデータを読み込む
// File wfr = SPIFFS.open(mapFileName, "r");

// for (int m = 0; m < roiW; m++) {
//     for (int n = 0; n < roiH; n++) {
...
//                 // sprite64cnos[(sy*8) * PNG_SPRITE_WIDTH + (sx*8) 取得スタート位置
//                 // y*PNG_SPRITE_WIDTH + x スプライトのピクセル取得
//                 sprpos = (sy * 8 * PNG_SPRITE_WIDTH + sx * 8 + y * PNG_SPRITE_WIDTH + x) / 2; // 4ビット二つで8ビットに入れてるので1/2に
//                 bit4 = sprite64cnos_vector[sprpos];
//                 if (x % 2 == 1)
...
//         sprite88_roi.setPivot(w / 2.0, h / 2.0);
//         sprite88_roi.pushSprite(&tft, roiy + m * 8, roix + n * 8); // 4ずれない(xy反転)
//     }
// }
...
return 0;
  ...
  //       // int sprno = getMapSprNo2csv(wfr, wx, wy);
  ...
  //           sprite88_roi.setPivot(w/2.0, h/2.0);
  //           // sprite88_roi.pushRotateZoom(&tft, mx, my, 0, 1, 1, TFT_BLACK);
  //           // sprite88_roi.pushSprite(&tft, roix+n*8, roiy+m*8);//4ずれない(png読み込みの場合はこっち）)
  //           sprite88_roi.pushSprite(&tft, roiy+m*8, roix+n*8);//4ずれない(xy反転)
  //     }
  // }
  // wfr.close();
...
int RunHaco8Game::l_print(lua_State* L){
  ...
  if(x == NULL && y == NULL && cn == NULL){//位置指定なしの場合
    ...
    // tft.setTextColor(HACO3_C7, HACO3_C0);//文字背景あり
